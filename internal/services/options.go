...
type Option func(*ClientApi)

func WithGraphQLClient(gql graphify.GraphQL) Option {
	return func(c *ClientApi) { c.gql = gql }
}
...
func WithToken(token string) Option {
	return func(c *ClientApi) { c.accessToken = token }
}

// WithPrivateAppAuth optionally sets private app credentials (API key and access token).
func WithPrivateAppAuth(apiKey, accessToken string) Option {
	return func(c *ClientApi) {
		c.apiKey = apiKey
